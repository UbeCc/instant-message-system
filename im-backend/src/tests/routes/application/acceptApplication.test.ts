...
describe("POST /api/request/accept", () => {
    ...
    describe("acceptFriendRequestHandler()", () => {
        it("should respond with successful message when they are friends", async () => {
            ...
            const db = await connectToDatabase();
            const acceptedApplication = await db.collection("friendshipApplication").findOne({ _id: testRequestID });
            expect(acceptedApplication?.status).toBe("accepted");
            const friendship = await db.collection("friendship").findOne({ sender: testUsername1, receiver: testUsername2 });
            expect(friendship).not.toBeNull();
        });
    });

    // 2. 接受入群申请
    describe("acceptGroupRequestHandler()", () => {
        // A. 群主审核，应当通过
        it("should respond with failed message when they are not friends", async () => {
            ...
            const db = await connectToDatabase();
            const acceptedApplication = await db.collection("groupApplication").findOne({ _id: testRequestID });
            expect(acceptedApplication?.status).toBe("accepted");
            const groupMember = await db.collection("groupMember").findOne({ username: testUsername1 });
            expect(groupMember).not.toBeNull();
            await db.collection("groupMember").deleteOne({ username: testUsername1 });
        });
    ...
    // 4. 异常处理
    it("should respond with \"Error Accepting Request. Error: ...\" when there is an error", async () => {
        const fastify = await initFastify();
        ...
        expect(response.json()).toEqual({
            code: -1,
            msg: "Error Accepting Request. Error: Invalid refresh token"
        });
    });
});