...
describe("POST /api/group/create", () => {
    ...
    it("should respond with \"Error Getting Content By Sender. Error: ...\" when there is an error", async () => {
        ...
        expect(response.json()).toEqual({
            code: -1,
            msg: "Error Getting Full Message. Error: Invalid refresh token",
            msgList: []
        });
    });

    // 4. 会话不存在
    it("should respond with \"Error Getting Full Message. Error: ...\" when there is an error", async () => {
        const fastify = await initFastify();
        ...
        expect(response.json()).toEqual({
            code: -1,
            msg: "Error Getting Full Message. Error: Error Getting Full Message List In Model. Error: The conversation doesn't exist.",
            msgList: []
        });
    });
});